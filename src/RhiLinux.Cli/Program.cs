...
internal static class CliApplication
{
    ...
    public static async Task<int> RunAsync(string[] args)
    {
        try
        {
            ...
            var sourcesAction = command == "sources" && args.Length > 1 && !args[1].StartsWith("--", StringComparison.Ordinal) ? args[1].ToLowerInvariant() : null;
            var options = Options.Parse(args[(cacheAction is null && sourcesAction is null ? 1 : 2)..]);
            ...
            var discovery = new SteamDiscoveryService(
            ...
            if (command == "scan")
            {
                if (options.Has("all-sources") || options.Value("provider") is not null)
                {
                    var enabled = ParseProviders(options.Value("provider"));
                    var multi = await multiSource.ScanAsync(new MultiSourceScanRequest(
                        EnabledProviders: enabled,
                        ForceFullScan: options.Has("force"),
                        SourceIndexPath: xdg.SourceIndexFile));
                    state.DiscoveredGames = ToPersisted(multi.Games);
                    state.LastScanUtc = DateTimeOffset.UtcNow;
                    if (!options.Has("no-save") && !diagnosticsReadOnly) await store.SaveAsync(state);
                    Write(multi, options.Has("json"), PrintMultiSource);
                    return multi.Diagnostics.Any(d => d.Severity == SourceDiagnosticSeverity.Error) ? 2 : 0;
                }

                var roots = options.Values("steam-root");
                var result = await discovery.ScanAsync(roots.Count == 0 ? null : roots, ToSteamAppOverrides(state.Overrides),
                    includeDefaultRoots: roots.Count > 0);
                state.DiscoveredGames = ToPersisted(result.Games); state.LastScanUtc = DateTimeOffset.UtcNow;
                if (!options.Has("no-save") && !diagnosticsReadOnly) await store.SaveAsync(state);
            ...
            {
                var multi = await multiSource.ScanAsync(new MultiSourceScanRequest(
                    EnabledProviders: ParseProviders(options.Value("provider")),
                    ForceFullScan: options.Has("force"),
                    SourceIndexPath: xdg.SourceIndexFile,
                    LibraryDatabasePath: xdg.LibraryDatabaseFile));
                currentScan = new ScanResult(
                    multi.Games,
                    [], [],
                    multi.Diagnostics.Where(d => d.Severity != SourceDiagnosticSeverity.Info).Select(d => d.Message).ToArray());
            }
            else
            {
                currentScan = await discovery.ScanAsync(configuredRoots.Count == 0 ? null : configuredRoots,
                    ToSteamAppOverrides(state.Overrides),
                    includeDefaultRoots: configuredRoots.Count > 0);
            }
            state.DiscoveredGames = ToPersisted(currentScan.Games);
            state.LastScanUtc = DateTimeOffset.UtcNow;
            if (!options.Has("no-save") && !diagnosticsReadOnly) await store.SaveAsync(state);
            var installGames = ToInstalledGames(state.DiscoveredGames);
            if (command == "games")
            {
                if (options.Has("json"))
                {
                    var payload = installGames.Select(ToInstallJson).ToArray();
                    Console.WriteLine(JsonSerializer.Serialize(payload, JsonOptions));
                }
                else PrintGames(installGames);
                return 0;
            }

            var game = SelectGame(installGames, options.Value("game"));
            if (options.Value("exe") is { } executableOverride)
            {
                state.Overrides[game.EffectiveInstallId] = new GameOverride(executableOverride, options.Value("deployment-dir"));
                if (!diagnosticsReadOnly) await store.SaveAsync(state);
                var roots = options.Values("steam-root");
                var refreshed = await discovery.ScanAsync(roots.Count == 0 ? [game.SteamRoot ?? string.Empty] : roots,
                    ToSteamAppOverrides(state.Overrides));
                game = refreshed.Games.Single(x => x.EffectiveInstallId == game.EffectiveInstallId ||
                    (game.SteamAppId is { } steamAppId && x.AppId == steamAppId));
                state.DiscoveredGames.RemoveAll(x => x.InstallId == game.EffectiveInstallId);
                state.DiscoveredGames.Add(PersistedGameEntry.FromInstalledGame(game));
                if (!diagnosticsReadOnly) await store.SaveAsync(state);
            }
            ...
            {
                case "status":
                    var statuses = await new ComponentDetector().DetectAsync(target);
                ...
                case "profile":
                    var profile = await new GameProfileCatalog(xdg).MatchAsync(target);
                ...
                case "proxy-diagnostics":
                    var diagnostics = await new ProxyDiagnosticsService(new GameProfileCatalog(xdg)).DiagnoseAsync(target);
                    Write(diagnostics, options.Has("json"), value => PrintProxyDiagnostics(game, value));
                    return diagnostics.HasSafeProxy ? 0 : 3;
                case "artifacts":
                    var artifacts = await new OfficialArtifactResolver(new HttpClient(), xdg).ResolveAsync(target, !options.Has("offline"));
    ...
    private static async Task<int> HandleSources(string? action, Options options, XdgPaths xdg)
    {
        ...
        if (action == "list")
        {
            ...
            foreach (var provider in providers)
            {
                ...
                {
                    Console.WriteLine($"{provider.Id,-12} {provider.DisplayName}");
                    foreach (var root in roots)
                        Console.WriteLine(
                            $"  {(root.Exists ? "ok" : "missing"),-8} {root.Kind,-10} {root.CanonicalPath}" +
    ...
    private static object ToInstallJson(InstalledGame game) => new
    {
        installId = game.EffectiveInstallId,
        name = game.Name,
        store = game.Store.ToString(),
        launcher = game.PrimaryLauncher.ToString(),
        externalId = game.ExternalId,
        steamAppId = game.SteamAppId,
        installRoot = game.GameRoot,
        executable = game.Executable,
        prefix = string.IsNullOrWhiteSpace(game.ProtonPrefix) ? null : game.ProtonPrefix,
        deploymentDirectory = game.DeploymentDirectory,
        stale = game.IsStale,
        platform = game.Platform.ToString(),
        sources = game.Sources.Select(source => new
        {
            source.ProviderId,
            store = source.Store.ToString(),
            launcher = source.Launcher.ToString(),
            source.ExternalId,
            source.MetadataPath
        }),
    ...
    private static void PrintMultiSource(MultiSourceScanResult multi)
    {
        Console.WriteLine(
            $"Providers: {multi.ProviderResults.Count}; installs: {multi.Games.Count}; " +
            $"duplicates: {multi.Reconciliation.DuplicatePhysicalInstallCount}; generation: {multi.ScanGeneration}");
        foreach (var game in multi.Games)
            Console.WriteLine(
                $"{game.Store,-7} {game.PrimaryLauncher,-10} {game.ExternalId ?? game.InstallId.Value,-24} {game.Name} | " +
                $"{game.Executable ?? "no executable"}");
        foreach (var timing in multi.TimingsMilliseconds)
            Console.WriteLine($"timing {timing.Key}: {timing.Value:0.0} ms");
        foreach (var diagnostic in multi.Diagnostics.Where(item => item.Severity != SourceDiagnosticSeverity.Info).Take(20))
            Console.Error.WriteLine($"warning: [{diagnostic.ProviderId}/{diagnostic.Code}] {diagnostic.Message}");
    }

    private static async Task<int> HandleInstallLike(string command, InstalledGame game, Options options, ApplicationState state, IStateStore store)
    {
        if (command == "plan" && options.Has("recommended"))
        {
            ...
            var target = game.ToDeploymentTarget();
            var resolution = await new OfficialArtifactResolver(new HttpClient(), paths, catalog).AcquireAsync(target, !options.Has("offline"));
            var proxy = await new ProxyDiagnosticsService(catalog).DiagnoseAsync(target);
            var recommendedPlan = await new DeploymentPlanner().BuildRecommendedStackPlanAsync(target,
    ...
    private static async Task<int> HandleRemove(InstalledGame game, Options options, ApplicationState state, IStateStore store)
    {
        var component = ParseComponent(options.Required("component"));
        var plan = await new DeploymentPlanner().BuildRemovePlanAsync(game.ToDeploymentTarget(), component, options.Value("proxy") ?? "dxgi.dll");
        return await ExecutePlan(plan, options, state, store);
    }
    ...
    private static async Task<int> PrintStackStatus(InstalledGame game, Options options, XdgPaths paths)
    {
        var report = await new StackStatusService(new HttpClient(), paths).GetAsync(game.ToDeploymentTarget(), !options.Has("offline"));
    ...
    private static async Task<int> PrintDiagnostics(InstalledGame game, Options options, XdgPaths paths)
    {
        var allowNetwork = !options.Has("offline");
        using var httpClient = new HttpClient();
        var statusService = new StackStatusService(httpClient, paths);
        var target = game.ToDeploymentTarget();
        var report = await statusService.GetAsync(target, allowNetwork);
        ...
        if (options.Has("download"))
        {
            ...
            report = await statusService.GetAsync(target, false);
        ...
        {
            Console.WriteLine($"Release diagnostics for {game.Name} ({game.EffectiveInstallId})");
            PrintProfile(game, report.Profile);
            PrintProxyDiagnostics(game, report.Proxy);
    ...
    private static async Task<int> ExecutePlan(DeploymentPlan plan, Options options, ApplicationState state, IStateStore store)
    {
        ...
        state.Transactions.Add(new TransactionRecord(plan.Id, plan.InstallId, plan.Action, started, DateTimeOffset.UtcNow, result.RolledBack, result.Messages, result.Error, plan.SteamAppId));
    ...
    private static List<InstalledGame> ToInstalledGames(IEnumerable<PersistedGameEntry> entries) =>
        entries.Select(InstalledGame.FromPersistedGameEntry).ToList();

    private static List<PersistedGameEntry> ToPersisted(IEnumerable<InstalledGame> games) =>
        games.Select(PersistedGameEntry.FromInstalledGame).ToList();

    private static InstalledGame SelectGame(IReadOnlyList<InstalledGame> games, string? selector)
    {
        ...
        var matches = games.Where(x =>
            string.Equals(x.EffectiveInstallId, selector, StringComparison.OrdinalIgnoreCase) ||
            string.Equals(x.ExternalId, selector, StringComparison.OrdinalIgnoreCase) ||
            (uint.TryParse(selector, out var appId) && x.AppId == appId) ||
            x.Name.Contains(selector, StringComparison.OrdinalIgnoreCase)).ToArray();
        if (matches.Length > 1 && uint.TryParse(selector, out var exactAppId))
            matches = matches.Where(x => x.AppId == exactAppId).ToArray();
        if (matches.Length > 1)
            matches = matches.Where(x =>
                string.Equals(x.EffectiveInstallId, selector, StringComparison.OrdinalIgnoreCase) ||
                string.Equals(x.ExternalId, selector, StringComparison.OrdinalIgnoreCase) ||
                (uint.TryParse(selector, out var id) && x.AppId == id)).ToArray();
    ...
    private static void PrintGames(IReadOnlyList<InstalledGame> games)
    {
        foreach (var game in games)
            Console.WriteLine(
                $"{game.StoreBadge,-7} {game.LauncherBadge,-10} {game.ExternalId ?? game.SteamAppId?.ToString() ?? game.EffectiveInstallId,-12} {game.Name} | " +
                $"{game.Engine} | {game.Confidence} | {game.Executable}");
    }
    private static void PrintCandidates(InstalledGame game, IReadOnlyList<ExecutableCandidate> candidates)
    {
        Console.WriteLine($"{game.Name} ({game.EffectiveInstallId})");
    ...
    private static void PrintStatuses(InstalledGame game, IReadOnlyList<ComponentStatus> statuses)
    {
        Console.WriteLine($"{game.Name} ({game.EffectiveInstallId}) — {game.DeploymentDirectory}");
        foreach (var status in statuses) Console.WriteLine($"{status.Component,-12} {status.Health,-11} {status.Explanation}");
    }
    private static void PrintProfile(InstalledGame game, GameProfileMatch match)
    {
        Console.WriteLine($"{game.Name} ({game.EffectiveInstallId}) — profile {match.Profile.Id}");
    ...
    private static void PrintProxyDiagnostics(InstalledGame game, ProxySelectionResult result)
    {
        Console.WriteLine($"Proxy diagnostics for {game.Name} ({game.EffectiveInstallId})");
    ...
    private static void PrintArtifacts(InstalledGame game, GameArtifactResolution resolution)
    {
        Console.WriteLine($"Official artifacts for {game.Name} ({game.EffectiveInstallId})");
    ...
    {
        Console.WriteLine($"Plan {plan.Id}: {plan.Action} for install {plan.InstallId}" +
            (plan.SteamAppId is { } steamAppId ? $" (Steam AppID {steamAppId})" : string.Empty));
    ...
    private static void PrintHelp() => Console.WriteLine("""
        ...
        Commands:
          ...
          sources list [--json]
          sources scan [--provider heroic] [--force] [--json]
          games [--all-sources] [--provider steam,heroic] [--json]
